...
#define CODEC_ADDR					0x1A // equal to 0b0011010
...
XDmaPs_Cmd DmaRead;
XDmaPs_Cmd DmaWrite;
static unsigned int AudioInputBuffer[AUDIO_BUFFER_LEN];
static unsigned int AudioOutputBuffer[AUDIO_BUFFER_LEN];
...
void AudioInputHandler(void *)
{
	...
	XDmaPs_SetDoneHandler(&DmaInst, DmaChannel, (XDmaPsDoneHandler) DmaReadDoneHandler, (void *) AudioInputBuffer);
...
void AudioOutputHandler(void *)
{
	...
	XScuGic_Disable(&IntrInst, AUDIO_INPUT_BUFFER_INTR);
	XScuGic_Disable(&IntrInst, AUDIO_OUTPUT_BUFFER_INTR);
	XDmaPs_SetDoneHandler(&DmaInst, DmaChannel, (XDmaPsDoneHandler) DmaWriteDoneHandler, (void *) AudioOutputBuffer);
...
{
	for (u16 idx = 0; idx < AUDIO_BUFFER_LEN; idx++) {
		AudioOutputBuffer[idx] = AudioInputBuffer[idx];
	}
...
XStatus codec_write_reg(u8 Address, u16 Data)
{
	...
	Status = XIicPs_MasterSendPolled(&IICInst, u8TxData, 2, CODEC_ADDR);
	if (Status != XST_SUCCESS) {
...
XStatus codec_read_reg(u8 Address, u16 *Data)
{
	...
	Status = XIicPs_MasterSendPolled(&IICInst, u8TxData, 1, CODEC_ADDR);
	if (Status != XST_SUCCESS) {
	   xil_printf("IIC write to address 0x%08x failed\r\n", u8TxData);
	   return XST_FAILURE;
	}
	XIicPs_ClearOptions(&IICInst, XIICPS_REP_START_OPTION);    //enable sending STOP bit after data transfer
	Status = XIicPs_MasterRecvPolled(&IICInst, u8RxData, 2, CODEC_ADDR);
	if (Status != XST_SUCCESS) {
